---
title: "Women in Parliament - 1 commit"
...
```{r}
#Check the files types

#wip
#str(wip)
#class(wip)

#Check that values are NA (before removing)
...
```{r}
#Reshape the data & remove missing data (NA) with na.rm 

WP <- melt(wip,
           id.vars = c("Country", "Code"),
           measure = patterns("^X"),
           variable.name = c("YearC"),
           value.name = c("pctWiP"),
           na.rm = TRUE)
WP
```
...
```{r}
#Creating numeric Year variable and ratio of men to women in parliament
...
#Countrys in rows || Code - Year columns
```
...
```{r}
#Plot interpretation 
#In 1990 Portugal had 7,6% of women in parliament
# % of women in parliament increased over time, in 2018 the % is 34.8
#Still falls short of 50%
```
...
```
```{r}
#Which countries have the hieghest percentage of women in parliamnet by year?
